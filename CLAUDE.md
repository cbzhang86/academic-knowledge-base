...
先读论文 → 判断方向（养老服务/数字排斥） → 自动生成审稿背景包：

```bash
python "14_工具脚本/报告/ars_kb_bridge.py" stage-3 --topic "养老服务 数字排斥"
```
...
```bash
python "14_工具脚本/报告/ars_kb_bridge.py" stage-5 --refs "提取的引文关键词"
```
...
```bash
# 第1步：完整性审核（引文对照）
python "14_工具脚本/报告/ars_kb_bridge.py" stage-25 --topic "主题" --refs "参考文献"
# → 知识包注入 → ARS 跑 Stage-25 → 输出完整性审核报告

# 第2步：同行评审
python "14_工具脚本/报告/ars_kb_bridge.py" stage-3 --topic "主题"
# → 知识包+完整性报告 → ARS 跑 Stage-3 → 输出5份审稿报告+决策函

# 第3步：修订
python "14_工具脚本/报告/ars_kb_bridge.py" stage-4 --comments "审稿意见原文"
# → 知识包+审稿报告 → ARS 跑 Stage-4 → 输出修订稿+R&R答复信
```
