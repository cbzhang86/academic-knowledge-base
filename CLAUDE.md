...
#### 教训1：ARS skill 加载失败时的正确处理

ARS 通过 Skill 工具动态加载，**compact（上下文压缩）后注册信息会丢失**。再次调用 `/ars-full` 可能失败。

**正确做法**：
- 第一次失败后，应**重新调用 Skill 工具重试**，而非自行降级
- 如果仍然不可用，说明该对话窗口中 ARS 注册已丢失，**应由用户重新触发 `/ars-full`**
- **严禁**用自己的子智能体代替 ARS 的多 Agent 审稿机制——普通 Agent 没有 ARS 审稿人的专业提示词（EIC/方法论专家/魔鬼代言人等角色定义），质量不在一个级别

**替代方案（当 ARS 确实不可用时）**：
- 轻量级路径（手动逐阶段推进 + 知识注入）是可行的，但前提是**告诉用户当前走的是降级路径，输出的审稿质量低于 ARS 标准**
- 验证环节必须由独立 Agent 从零开始检查，且在该 Agent 的 prompt 中明确要求"检查所有统计量的来源是否真实可追溯"
