...
如果你只有初稿需要审核，不想跑完整 ARS 管线，可以用轻量级路径：

```bash
# 第1步：生成审稿背景知识包
python "14_工具脚本/报告/ars_kb_bridge.py" stage-3 \
  --topic "论文主题关键词"

# 第2步：生成引文审计包
python "14_工具脚本/报告/ars_kb_bridge.py" stage-5 \
  --refs "参考文献文件路径或逗号分隔"
```

然后在对话中发送：

> 我已有一篇论文初稿，路径是【论文路径】。请分三步完成：
>
> **第一步：全面审稿**
> 1. 先读取 `10_研究输出/ars_output/` 中最新的知识包文件（审稿背景）
> 2. 然后以学术审稿人身份审读论文全文
> 3. 输出 5 份审稿报告：理论框架评价、方法评价、论证逻辑、创新性、写作质量
>
> **第二步：引文审计**
> 1. 读取引文审计知识包
> 2. 检查论文引文是否覆盖了知识库中相关的高评分文献
> 3. 如有遗漏，建议补充
>
> **第三步：修订建议**
> 1. 综合审稿意见和引文审计结果
> 2. 给出具体的修订建议（按重要性排序）
> 3. 每项建议附带：原文位置 + 修改方案 + 预期效果
...
> /ars-full 使用这篇论文初稿跑全流程：审稿→修订→定稿。
> 论文路径：【论文路径】
> 注意：
> 1. 论文正文必须保持完整的学术论文字数（约8000-12000字）
> 2. 审稿阶段前读取 ars_output/ 下的知识包作为审稿背景
> 3. 引文审计阶段读取引文审计知识包做交叉验证
> 4. 最终定稿输出到 d:\公共管理科研\10_研究输出\定稿\
